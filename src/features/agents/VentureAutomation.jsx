...
import { Rocket, Target, Bell, FileBarChart, TrendingUp, Sparkles } from 'lucide-react';

export default function VentureAutomation() {
  ...
  return (
    <PageContainer>
      <PageHeader title="Venture Automation" description="AI-powered workflows for venture tracking, milestones, and reports." />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Venture automation workflows will appear here once configured."
        />
      ) : (
        <>
          {stats.length > 0 && (
          ...
          <div className="space-y-4">
            {capabilities.map((c, i) => (
              <div key={i} className="flex flex-col gap-4 rounded-xl border border-border bg-surface/40 p-5 backdrop-blur-sm sm:flex-row sm:items-center">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-pink-500/10 text-pink-400"><c.icon className="h-6 w-6" /></div>
