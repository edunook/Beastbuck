/**
 * Auto-regenerate portfolios every 2 hours
 * Run with: node scripts/autoRegeneratePortfolios.cjs
 * This script will keep running and regenerate portfolios every 2 hours
 */
...
async function regenerateAllPortfolios() {
  ...
  console.log(`[${timestamp}] Starting portfolio regeneration...`);
  ...
  try {
    ...
    console.log(`[${timestamp}] Portfolio regeneration complete:`);
    console.log(`✓ Successful: ${successful.length}`);
    console.log(`✗ Failed: ${failed.length}`);
    
    if (failed.length > 0) {
      console.log('Failed portfolios:');
      failed.forEach(f => {
    ...
    console.error(`[${timestamp}] Error regenerating portfolios:`, error);
  }
}
...
// Then run every 2 hours (2 * 60 * 60 * 1000 = 7,200,000 ms)
const TWO_HOURS = 2 * 60 * 60 * 1000;
setInterval(() => {
  regenerateAllPortfolios();
}, TWO_HOURS);

console.log('Auto-regeneration started. Portfolios will be regenerated every 2 hours.');
