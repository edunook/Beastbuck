...
import { 
  ...
  MessageSquare, Archive, Star, Bell, Flame, Clock, TrendingUp,
  UserPlus, Bot, ArrowRight
} from 'lucide-react';
...
function getConversationIcon(room) {
  ...
  if (room.name?.toLowerCase() === 'general' || room.name?.toLowerCase() === 'community') return MessageSquare;
  return Hash;
}

function getConversationColor(room, isActive) {
  if (room.type === 'announcement') return {
    bg: 'from-orange-500/20 to-orange-500/10',
    border: 'border-orange-500/30',
    text: 'text-orange-400',
    activeBg: 'from-orange-500/30 to-orange-500/20',
    activeBorder: 'border-orange-500/50',
    activeShadow: 'shadow-orange-500/30'
  };
  if (isActive) return {
    bg: 'from-accent/30 to-accent/20',
    border: 'border-accent/40',
    text: 'text-accent',
    activeBg: 'from-accent/30 to-accent/20',
    activeBorder: 'border-accent/50',
    activeShadow: 'shadow-accent/30'
  };
  return {
    bg: 'from-white/10 to-white/5',
    border: 'border-white/10',
    text: 'text-white/70',
    activeBg: 'from-white/10 to-white/5',
    activeBorder: 'border-white/10',
    activeShadow: 'shadow-transparent'
  };
}
  ...
  return (
    <aside className="flex min-h-0 w-full shrink-0 flex-col border-b border-white/10 bg-gradient-to-b from-slate-900/80 to-slate-950/90 backdrop-blur-xl md:w-72 md:border-b-0 md:border-r md:border-white/5">
      ...
      <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-4 bg-gradient-to-r from-white/5 to-transparent">
        <div>
          <h2 className="text-sm font-bold uppercase tracking-widest text-white/90">Messages</h2>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {canManageChannels && (
            <button 
              ...
              aria-label={creating ? 'Close form' : 'New conversation'}
            >
      ...
      {/* Conversation List */}
      <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto p-2 md:p-3 custom-scrollbar" aria-label="Conversations">
        {sortedRooms.length === 0 ? (
          ...
          sortedRooms.map(room => {
            ...
            const colors = getConversationColor(room, active);
            const Icon = getConversationIcon(room);
              ...
              <div key={room.id} className="group relative">
                <button
                  ...
                  className={`relative flex w-full items-center gap-2.5 sm:gap-3 rounded-xl px-2.5 py-2.5 sm:px-3 text-left transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] ${
                    active
                      ? `border ${colors.activeBorder} bg-gradient-to-r ${colors.activeBg} text-white shadow-lg ${colors.activeShadow}`
                      : `border ${colors.border} text-white/70 hover:border-white/10 hover:bg-white/5 hover:text-white`
                  }`}
                >
                  {/* Active Indicator */}
                  {active && (
                  ...
                  <span className={`rounded-lg p-2 transition-all duration-200 bg-gradient-to-br ${active ? colors.activeBg : colors.bg} ${active ? colors.text : 'text-white/70'}`}>
                  ...
                  {unreadCount > 0 && !active && (
                    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-accent text-[10px] font-bold text-black shadow-sm animate-pulse">
