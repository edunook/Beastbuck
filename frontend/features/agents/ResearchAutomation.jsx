...
export default function ResearchAutomation() {
  ...
  return (
    <PageContainer>
      <PageHeader title="Research Automation" description="AI-powered workflows for literature, citations, and knowledge extraction." />
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Research automation workflows will appear here once configured."
        />
      ) : (
        <>
          {stats.length > 0 && (
          ...
          <div className="space-y-4">
            {capabilities.map((c, i) => (
              <div key={i} className="flex flex-col gap-4 rounded-xl border border-border bg-surface/40 p-5 backdrop-blur-sm sm:flex-row sm:items-center">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-accent/10 text-accent"><c.icon className="h-6 w-6" /></div>
                <div className="flex-1">
                  <h3 className="font-heading font-bold text-white">{c.title}</h3>
                  <p className="text-xs text-text-muted">{c.desc}</p>
                  <p className="mt-1 text-xs text-text-muted">Last run: {c.lastRun} · Results: {c.results}</p>
                </div>
